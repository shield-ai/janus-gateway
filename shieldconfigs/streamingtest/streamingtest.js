...
$(document).ready(function() {
	$('#ip-input').removeAttr('disabled');
	// Initialize the library (all console debuggers enabled)
	Janus.init({debug: "all", callback: function() {
		// Use a button to start the demo
		$('#start').click(function() {

			hostIP = document.getElementById("ip-input").value;
			server = [
				"ws://" + hostIP + ":8188",
				"http://" + hostIP + ":8088/janus",
				"https://" + hostIP + ":8089/janus"
			];
			$('#ip-input').attr('disabled', true);
						...
						janus.attach(
							{
								...
								onremotestream: function(stream) {
									...
									if($('#remotevideo').length === 0)
										$('#stream').append('<button class="btn btn-default" disabled="true" autocomplete="off" id="fullscreen">Fullscreen</button>\
		...
		$('#ip-input').keydown(function(e) {
			if (e.keyCode === 13 && document.getElementById("ip-input").value !== "") {
				$('#start').trigger('click');
			}
		});
